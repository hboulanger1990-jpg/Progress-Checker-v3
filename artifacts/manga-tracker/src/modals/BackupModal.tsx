...
export default function BackupModal({ data, onClose, onImport }: Props) {
  ...
  const tabClass = (t: "export" | "import") =>
    `flex-1 py-2 rounded-lg text-sm font-medium transition-all ${tab === t ? "bg-[var(--border)] text-[var(--text-primary)]" : "text-[var(--text-muted)]"}`;

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      <div className="absolute inset-0 bg-[--shadow-modal]/60 backdrop-blur-sm animate-fade-in" onClick={onClose} />
      <div className="relative w-full max-w-sm bg-[var(--bg-overlay)] border border-[var(--border)] rounded-t-2xl sm:rounded-2xl p-6 shadow-2xl animate-slide-up max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-[var(--text-primary)]">バックアップ</h2>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] text-xl leading-none transition-colors"><CloudUpload size={20} /></button>
        </div>
        <div className="flex gap-1 mb-4 bg-[var(--bg-surface)] rounded-xl p-1">
        ...
        {tab === "export" && (
          <div className="flex flex-col gap-3 flex-1 overflow-hidden">
            <p className="text-xs text-[var(--text-muted)]">全データをJSONとしてコピーし、メモ帳などに保管してください。</p>
        ...
        {tab === "import" && (
          <div className="flex flex-col gap-3 flex-1 overflow-hidden">
            <p className="text-xs text-[var(--text-muted)]">バックアップしたJSONファイルを選択するか、テキストを貼り付けてインポートします。</p>
            <input
              type="file"
              accept=".json"
              onChange={(e) => {
              ...
              className="text-xs text-[var(--text-muted)] file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-[var(--border)] file:text-[var(--text-primary)] file:text-xs"
            />
            <textarea
              value={importText}
